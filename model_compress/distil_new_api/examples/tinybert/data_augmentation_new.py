...
from bert_model.language_model import BertForMaskedLM
...
class DataAugmentor(object):
    ...
    def _masked_language_model(self, sent, word_pieces, mask_id):
        ...
        tokens_tensor = flow.tensor([token_ids]).to(device)
        segments_tensor = flow.tensor([segments_ids]).to(device)

        self.model.to(device)

        predictions = self.model(tokens_tensor, segments_tensor)

        word_candidates = flow.argsort(predictions[0, mask_id], descending=True)[:self.M].tolist()
